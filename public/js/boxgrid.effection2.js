...
******修改注释掉**********************/
	...
	function initEvents() {
		...
		$items.each( function() {
			...
			// var $item = $( this ),//////======设置只有点查看时才进图详细====================
			var $item = $( this ),
				$liItem = $item.parent(),//获取li元素
				// $close = $item.find( 'span.rb-close' ),
				// $overlay = $item.children( 'div.rb-overlay' );
				$close = $liItem.find( 'span.rb-close' ),
				$overlay = $liItem.children( 'div.rb-overlay' );
			...
			$item.on( 'click', function() {
				...
				var layoutProp = getItemLayoutProp( $liItem ),//======设置在li元素下剪取==========
			...
			$close.on( 'click', function() {
				...
				// var layoutProp = getItemLayoutProp( $item ),
				var layoutProp = getItemLayoutProp( $liItem ),//=========设置回到li元素的大小
					clipPropFirst = 'rect(' + layoutProp.top + 'px ' + ( layoutProp.left + layoutProp.width ) + 'px ' + ( layoutProp.top + layoutProp.height ) + 'px ' + layoutProp.left + 'px)',
					clipPropLast = 'auto';
